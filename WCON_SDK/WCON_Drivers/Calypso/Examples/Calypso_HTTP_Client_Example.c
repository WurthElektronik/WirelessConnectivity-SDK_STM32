...
void Calypso_HTTP_Client_Example(void)
{
    ...
    printf("*** Start of Calypso HTTP client example ***\r\n");
    ...
    if (secure)
    {
        ...
        ret = ATFile_Open(serverRootCertFile,
    ...
    /* Create HTTP client */
    ret = ATHTTP_Create(&clientHandle);
    Calypso_Examples_Print("Create HTTP client", ret);

    /* Connect to HTTP server */
    ret = ATHTTP_Connect(clientHandle,
                         hostWithPrefix,
                         ATHTTP_ConnectFlags_IgnoreProxy,
                         NULL,
                         NULL,
                         secure ? serverRootCertFile : "");
    ...
    printf("Sending \"%s\" to HTTP server...\r\n", payload);
    ret = ATHTTP_SendRequest(clientHandle,
                             ATHTTP_Method_Get,
                             url,
                             ATHTTP_RequestFlags_None,
                             dataFormat,
                             base64,
                             strlen(payload),
                             payload,
    ...
    printf("HTTP status code is %ld (%s)\r\n",
           status,
           status == 200 ? "OK" : "NOK");

    /* Read HTTP response body in chunks of length up to rxChunksize */
    ATHTTP_ResponseBody_t body;
    uint16_t chunkIndex = 0;
    do
    {
        ret = ATHTTP_ReadResponseBody(clientHandle,
                                      dataFormat,
                                      base64,
                                      rxChunkSize,
                                      &body);
        ...
        if (ret)
        {
            printf("Received HTTP response body chunk %d: \"%s\"\r\n",
                   chunkIndex++,
                   body.body);
        }
    }
    ...
    ret = ATHTTP_Disconnect(clientHandle);
    Calypso_Examples_Print("HTTP disconnect client", ret);

    /* Delete HTTP client */
    ret = ATHTTP_Destroy(clientHandle);
