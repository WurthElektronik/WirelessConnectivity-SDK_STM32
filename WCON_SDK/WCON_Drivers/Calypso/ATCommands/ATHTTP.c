...
static const char *Calypso_ATHTTP_ConnectFlags_Strings[Calypso_ATHTTP_ConnectFlags_NumberOfValues] = {
		"ignore_proxy",
		"host_exist" };

static const char *Calypso_ATHTTP_Method_Strings[Calypso_ATHTTP_Method_NumberOfValues] = {
		"get",
		"post",
		"head",
		"options",
...
static const char *Calypso_ATHTTP_RequestFlags_Strings[Calypso_ATHTTP_RequestFlags_NumberOfValues] = {
		"chunk_start",
		"chunk_end",
		"drop_body" };

static const char *Calypso_ATHTTP_HeaderField_Strings[Calypso_ATHTTP_HeaderField_NumberOfValues] = {
		"res_age",
		"res_allow",
		"res_cache_control",
		"res_connection",
...
static const char *Calypso_ATHTTP_HeaderPersistency_Strings[Calypso_ATHTTP_HeaderPersistency_NumberOfValues] = {
		"not_persistent",
		"persistent" };
...
bool Calypso_ATHTTP_Create(uint8_t *clientHandle)
{
	...
	return ATCommand_GetNextArgumentInt(&pRespondCommand, clientHandle,
...
bool Calypso_ATHTTP_Destroy(uint8_t clientHandle)
{
	...
	if (!ATCommand_AppendArgumentInt(pRequestCommand, clientHandle,
	ATCOMMAND_INTFLAGS_SIZE8 | ATCOMMAND_INTFLAGS_UNSIGNED | ATCOMMAND_INTFLAGS_NOTATION_DEC,
	ATCOMMAND_STRING_TERMINATE))
	{
		return false;
...
bool Calypso_ATHTTP_Connect(uint8_t clientHandle, const char *host, uint8_t flags, const char *privateKey, const char *certificate, const char *rootCaCertificate)
{
	...
	if (!ATCommand_AppendArgumentString(pRequestCommand, host,
	ATCOMMAND_ARGUMENT_DELIM))
	{
		return false;
	}
	if (!ATCommand_AppendArgumentBitmask(pRequestCommand, Calypso_ATHTTP_ConnectFlags_Strings, Calypso_ATHTTP_ConnectFlags_NumberOfValues, flags,
	ATCOMMAND_ARGUMENT_DELIM,
	AT_MAX_COMMAND_BUFFER_SIZE))
	{
		return false;
	}
	if (!ATCommand_AppendArgumentString(pRequestCommand, privateKey,
	ATCOMMAND_ARGUMENT_DELIM))
	{
		return false;
	}
	if (!ATCommand_AppendArgumentString(pRequestCommand, certificate,
	ATCOMMAND_ARGUMENT_DELIM))
	{
		return false;
	}
	if (!ATCommand_AppendArgumentString(pRequestCommand, rootCaCertificate,
	ATCOMMAND_STRING_TERMINATE))
	{
		return false;
	}
 ...
 * @return true if successful, false otherwise
 */
bool Calypso_ATHTTP_SendRequest(uint8_t clientHandle, Calypso_ATHTTP_Method_t method, const char *uri, uint8_t flags, Calypso_DataFormat_t format,
bool encodeAsBase64, uint16_t length, const char *data, uint32_t *status)
{
	*status = 0;
	...
	if (encodeAsBase64)
	{
		...
		Calypso_EncodeBase64((uint8_t*) data, length, (uint8_t*) base64Buffer, &lengthEncoded);
	...
	if (!ATCommand_AppendArgumentString(pRequestCommand, Calypso_ATHTTP_Method_Strings[method],
	ATCOMMAND_ARGUMENT_DELIM))
	{
		return false;
	}
	if (!ATCommand_AppendArgumentString(pRequestCommand, uri,
	ATCOMMAND_ARGUMENT_DELIM))
	{
		return false;
	}
	if (!ATCommand_AppendArgumentBitmask(pRequestCommand, Calypso_ATHTTP_RequestFlags_Strings, Calypso_ATHTTP_RequestFlags_NumberOfValues, flags,
	ATCOMMAND_ARGUMENT_DELIM,
	AT_MAX_COMMAND_BUFFER_SIZE))
	{
		return false;
	...
	return ATCommand_GetNextArgumentInt(&pRespondCommand, status,
 ...
 * @return true if successful, false otherwise
 */
bool Calypso_ATHTTP_ReadResponseBody(uint8_t clientHandle, Calypso_DataFormat_t format,
bool decodeBase64, uint16_t length, Calypso_ATHTTP_ResponseBody_t *responseBody)
{
	...
	if (!ATCommand_GetNextArgumentInt(&pRespondCommand, &responseBody->clientHandle,
	ATCOMMAND_INTFLAGS_SIZE8 | ATCOMMAND_INTFLAGS_UNSIGNED | ATCOMMAND_INTFLAGS_NOTATION_DEC,
	ATCOMMAND_ARGUMENT_DELIM))
	{
		return false;
	}
	if (!ATCommand_GetNextArgumentInt(&pRespondCommand, &responseBody->hasMoreData,
	ATCOMMAND_INTFLAGS_SIZE8 | ATCOMMAND_INTFLAGS_UNSIGNED | ATCOMMAND_INTFLAGS_NOTATION_DEC,
	ATCOMMAND_ARGUMENT_DELIM))
	{
		return false;
	}
	if (!ATCommand_GetNextArgumentInt(&pRespondCommand, &responseBody->format,
	ATCOMMAND_INTFLAGS_SIZE8 | ATCOMMAND_INTFLAGS_UNSIGNED | ATCOMMAND_INTFLAGS_NOTATION_DEC,
	ATCOMMAND_ARGUMENT_DELIM))
	{
		return false;
	}
	if (!ATCommand_GetNextArgumentInt(&pRespondCommand, &responseBody->length,
	...
	if (responseBody->length > 0)
	{
		if (decodeBase64)
		{
			...
			return Calypso_DecodeBase64((uint8_t*) pRespondCommand, encodedSize, (uint8_t*) responseBody->body, &decodedSize);
		}
		else
		{
			...
			return ATCommand_GetNextArgumentString(&pRespondCommand, responseBody->body,
			ATCOMMAND_STRING_TERMINATE, sizeof(responseBody->body));
		}
	}
 ...
 * @return true if successful, false otherwise
 */
bool Calypso_ATHTTP_SetHeader(uint8_t clientHandle, Calypso_ATHTTP_HeaderField_t field, Calypso_ATHTTP_HeaderPersistency_t persistency, Calypso_DataFormat_t format,
bool encodeAsBase64, uint16_t length, const char *data)
{
	if (encodeAsBase64)
	{
		/* Encode as Base64 */
	...
	if (!ATCommand_AppendArgumentString(pRequestCommand, Calypso_ATHTTP_HeaderField_Strings[field],
	ATCOMMAND_ARGUMENT_DELIM))
	{
		return false;
	}
	if (!ATCommand_AppendArgumentString(pRequestCommand, Calypso_ATHTTP_HeaderPersistency_Strings[persistency],
	ATCOMMAND_ARGUMENT_DELIM))
	{
		return false;
	}
 ...
 * @return true if successful, false otherwise
 */
bool Calypso_ATHTTP_GetHeader(uint8_t clientHandle, Calypso_ATHTTP_HeaderField_t field, Calypso_DataFormat_t format,
bool decodeBase64, uint16_t length, Calypso_ATHTTP_HeaderData_t *header)
{
	char *pRequestCommand = AT_commandBuffer;
	char *pRespondCommand = AT_commandBuffer;
	...
	if (!ATCommand_GetNextArgumentInt(&pRespondCommand, &header->clientHandle,
	ATCOMMAND_INTFLAGS_SIZE8 | ATCOMMAND_INTFLAGS_UNSIGNED | ATCOMMAND_INTFLAGS_NOTATION_DEC,
	ATCOMMAND_ARGUMENT_DELIM))
	{
		return false;
	}
	if (!ATCommand_GetNextArgumentInt(&pRespondCommand, &header->format,
	ATCOMMAND_INTFLAGS_SIZE8 | ATCOMMAND_INTFLAGS_UNSIGNED | ATCOMMAND_INTFLAGS_NOTATION_DEC,
	ATCOMMAND_ARGUMENT_DELIM))
	{
		return false;
	}
	if (!ATCommand_GetNextArgumentInt(&pRespondCommand, &header->length,
	ATCOMMAND_INTFLAGS_SIZE16 | ATCOMMAND_INTFLAGS_UNSIGNED | ATCOMMAND_INTFLAGS_NOTATION_DEC,
	ATCOMMAND_ARGUMENT_DELIM))
	{
		return false;
	...
	if (header->length > 0)
	{
		if (decodeBase64)
		{
			...
			return Calypso_DecodeBase64((uint8_t*) pRespondCommand, encodedSize, (uint8_t*) header->data, &decodedSize);
		}
		else
		{
			...
			return ATCommand_GetNextArgumentString(&pRespondCommand, header->data,
			ATCOMMAND_STRING_TERMINATE, sizeof(header->data));
		}
	}
 ...
 * @return true if successful, false otherwise
 */
bool Calypso_ATHTTP_SendCustomResponse(Calypso_DataFormat_t format,
bool encodeAsBase64, uint16_t length, const char *data)
{
	if (encodeAsBase64)
	{
