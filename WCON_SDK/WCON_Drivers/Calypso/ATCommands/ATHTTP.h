...
extern bool Calypso_ATHTTP_SendRequest(uint8_t clientHandle, Calypso_ATHTTP_Method_t method, const char *uri, uint8_t flags, Calypso_DataFormat_t format,
bool encodeAsBase64, uint16_t length, const char *data, uint32_t *status);
extern bool Calypso_ATHTTP_ReadResponseBody(uint8_t clientHandle, Calypso_DataFormat_t format,
bool decodeBase64, uint16_t length, Calypso_ATHTTP_ResponseBody_t *responseBody);
extern bool Calypso_ATHTTP_SetHeader(uint8_t clientHandle, Calypso_ATHTTP_HeaderField_t field, Calypso_ATHTTP_HeaderPersistency_t persistency, Calypso_DataFormat_t format,
bool encodeAsBase64, uint16_t length, const char *data);
extern bool Calypso_ATHTTP_GetHeader(uint8_t clientHandle, Calypso_ATHTTP_HeaderField_t field, Calypso_DataFormat_t format,
bool decodeBase64, uint16_t length, Calypso_ATHTTP_HeaderData_t *header);
extern bool Calypso_ATHTTP_SendCustomResponse(Calypso_DataFormat_t format,
bool encodeAsBase64, uint16_t length, const char *data);
