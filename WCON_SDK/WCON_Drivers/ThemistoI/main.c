...
#include "../../WCON_Drivers/ThemistoI/ThemistoI.h"
...
int main(void)
{
    ...
    ThemistoI_Init(115200, WE_FlowControl_NoFlowControl, AddressMode_0, RxCallback);

    while (1)
    {
        uint8_t version[3];
        memset(version,0,sizeof(version));
        ret = ThemistoI_GetFirmwareVersion(version);
        WE_Delay(500);

        uint8_t SN[4];
        memset(SN,0,sizeof(SN));
        ret = ThemistoI_GetSerialNumber(SN);
        WE_Delay(500);
