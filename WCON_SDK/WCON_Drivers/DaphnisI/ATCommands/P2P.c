...
bool DaphnisI_P2P_TransmitMulticastExtended(uint8_t group_id, uint8_t *payloadP, uint16_t payload_length)
{
	...
	if (!ATCommand_AppendArgumentInt(pRequestCommand, group_id,
	ATCOMMAND_INTFLAGS_NOTATION_DEC | ATCOMMAND_INTFLAGS_SIZE8 | ATCOMMAND_INTFLAGS_UNSIGNED, ATCOMMAND_ARGUMENT_DELIM))
	{
		return false;
	}
...
bool DaphnisI_P2P_SetRXEnabled(bool rx_state)
{
	...
	if (!ATCommand_AppendArgumentInt(pRequestCommand, (uint8_t)rx_state,
...
bool DaphnisI_P2P_GetRXEnabled(bool *rx_stateP)
{
	...
	return ATCommand_GetNextArgumentInt(&pRespondCommand, (uint8_t *)rx_stateP,
...
bool DaphnisI_P2P_ParseTXTimeEvent(char **pEventArguments, uint32_t *timeonairP)
{
	return ATCommand_GetNextArgumentInt(pEventArguments, timeonairP,
...
bool DaphnisI_P2P_ParseRXDataEvent(char **pEventArguments, DaphnisI_P2P_RxData_t *rxDataP)
{
	...
	if(!ATCommand_GetNextArgumentInt(pEventArguments, &(rxDataP->RSSI),
	...
	if(!ATCommand_GetNextArgumentInt(pEventArguments, &(rxDataP->data_length),
