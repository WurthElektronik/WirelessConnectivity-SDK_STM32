...
bool DaphnisI_GPIO_Local_Value_Get(DaphnisI_GPIO_t *gpioToReadP, uint8_t gpioToReadCount, DaphnisI_GPIOValueGetBlock_t *valueGetBlocksP)
{
	...
	{
		if (!ATCommand_AppendArgumentInt(pRequestCommand, gpioToReadP[i],
			(ATCOMMAND_INTFLAGS_NOTATION_DEC | ATCOMMAND_INTFLAGS_SIZE8 | ATCOMMAND_INTFLAGS_UNSIGNED),
...
bool DaphnisI_P2P_GPIO_Remote_ParseCfgChangedEvent(char **pEventArguments, DaphnisI_P2P_GPIO_RemoteCfgChangedData_t *cfgChangedP)
{
	...
	if(!ATCommand_GetNextArgumentInt(pEventArguments, &(cfgChangedP->RSSI),
	ATCOMMAND_INTFLAGS_SIZE16 | ATCOMMAND_INTFLAGS_SIGNED | ATCOMMAND_INTFLAGS_NOTATION_DEC,
...
bool DaphnisI_P2P_GPIO_Remote_ParseValueChangedEvent(char **pEventArguments, DaphnisI_P2P_GPIO_RemoteValueChangedData_t *valueChangedP)
{
	...
	if(!ATCommand_GetNextArgumentInt(pEventArguments, &(valueChangedP->RSSI),
	ATCOMMAND_INTFLAGS_SIZE16 | ATCOMMAND_INTFLAGS_SIGNED | ATCOMMAND_INTFLAGS_NOTATION_DEC,
	ATCOMMAND_ARGUMENT_DELIM))
	{
		return false;
...
bool DaphnisI_P2P_GPIO_Remote_ParseCfgSetResponseEvent(char **pEventArguments, DaphnisI_P2P_GPIO_RemoteCfgSetResponseData_t *cfgSetResponseP)
{
	...
	if(!ATCommand_GetNextArgumentInt(pEventArguments, &(cfgSetResponseP->RSSI),
	ATCOMMAND_INTFLAGS_SIZE16 | ATCOMMAND_INTFLAGS_SIGNED | ATCOMMAND_INTFLAGS_NOTATION_DEC,
	ATCOMMAND_ARGUMENT_DELIM))
	{
		return false;
...
bool DaphnisI_P2P_GPIO_Remote_ParseCfgGetResponseEvent(char **pEventArguments, DaphnisI_P2P_GPIO_RemoteCfgGetResponseData_t *cfgGetResponseP)
{
	...
	if(!ATCommand_GetNextArgumentInt(pEventArguments, &(cfgGetResponseP->RSSI),
	ATCOMMAND_INTFLAGS_SIZE16 | ATCOMMAND_INTFLAGS_SIGNED | ATCOMMAND_INTFLAGS_NOTATION_DEC,
	ATCOMMAND_ARGUMENT_DELIM))
	{
		return false;
	...
	if(!ATCommand_GetNextArgumentInt(pEventArguments, &(cfgGetResponseP->command_status),
...
bool DaphnisI_P2P_GPIO_Remote_ParseValueSetResponseEvent(char **pEventArguments, DaphnisI_P2P_GPIO_RemoteValueSetResponseData_t *valueSetResponseP)
{
	...
	if(!ATCommand_GetNextArgumentInt(pEventArguments, &(valueSetResponseP->RSSI),
	ATCOMMAND_INTFLAGS_SIZE16 | ATCOMMAND_INTFLAGS_SIGNED | ATCOMMAND_INTFLAGS_NOTATION_DEC,
	ATCOMMAND_ARGUMENT_DELIM))
	{
		return false;
	...
	if(!ATCommand_GetNextArgumentInt(pEventArguments, &(valueSetResponseP->command_status),
	ATCOMMAND_INTFLAGS_SIZE8 | ATCOMMAND_INTFLAGS_UNSIGNED | ATCOMMAND_INTFLAGS_NOTATION_DEC,
	ATCOMMAND_ARGUMENT_DELIM))
	{
		return false;
...
bool DaphnisI_P2P_GPIO_Remote_ParseValueGetResponseEvent(char **pEventArguments, DaphnisI_P2P_GPIO_RemoteValueGetResponseData_t *valueGetResponseP)
{
	...
	if(!ATCommand_GetNextArgumentInt(pEventArguments, &(valueGetResponseP->RSSI),
	ATCOMMAND_INTFLAGS_SIZE16 | ATCOMMAND_INTFLAGS_SIGNED | ATCOMMAND_INTFLAGS_NOTATION_DEC,
	ATCOMMAND_ARGUMENT_DELIM))
	{
		return false;
	...
	if(!ATCommand_GetNextArgumentInt(pEventArguments, &(valueGetResponseP->command_status),
	ATCOMMAND_INTFLAGS_SIZE8 | ATCOMMAND_INTFLAGS_UNSIGNED | ATCOMMAND_INTFLAGS_NOTATION_DEC,
	ATCOMMAND_ARGUMENT_DELIM))
	{
		return false;
...
static bool DaphnisI_GPIO_ParseConfigBlocks(char *string, DaphnisI_GPIOConfigBlock_t *configBlocksP, uint8_t *configBlocksCountP)
{
	...
	*configBlocksCountP = ATCommand_CountArgs(string);
	...
	{
		if(!ATCommand_GetNextArgumentInt(&string, &configBlocksP[i].GPIO_ID,
			ATCOMMAND_INTFLAGS_SIZE8 | ATCOMMAND_INTFLAGS_UNSIGNED | ATCOMMAND_INTFLAGS_NOTATION_DEC,
			GPIO_PIN_DELIMITER))
		{
			return false;
		...
		switch(configBlocksP[i].function)
		{
			...
			{
				if(!ATCommand_GetNextArgumentInt(&string, &(configBlocksP[i].value.input),
					ATCOMMAND_INTFLAGS_SIZE8 | ATCOMMAND_INTFLAGS_UNSIGNED | ATCOMMAND_INTFLAGS_NOTATION_DEC,
					value_delimiter))
				{
					return false;
			...
			{
				if(!ATCommand_GetNextArgumentInt(&string, &(configBlocksP[i].value.output),
					ATCOMMAND_INTFLAGS_SIZE8 | ATCOMMAND_INTFLAGS_UNSIGNED | ATCOMMAND_INTFLAGS_NOTATION_DEC,
					value_delimiter))
				{
					return false;
...
static bool DaphnisI_GPIO_ParseValueGetBlocks(char *string, DaphnisI_GPIOValueGetBlock_t *valueGetBlocksP, uint8_t *valueGetBlocksCountP)
{
	...
	*valueGetBlocksCountP = ATCommand_CountArgs(string);
	...
	{
		if(!ATCommand_GetNextArgumentInt(&string, &valueGetBlocksP[i].GPIO_ID,
			ATCOMMAND_INTFLAGS_SIZE8 | ATCOMMAND_INTFLAGS_UNSIGNED | ATCOMMAND_INTFLAGS_NOTATION_DEC,
			GPIO_PIN_DELIMITER))
		{
			return false;
		...
		if(!ATCommand_GetNextArgumentInt(&string, &(valueGetBlocksP[i].value),
			ATCOMMAND_INTFLAGS_SIZE8 | ATCOMMAND_INTFLAGS_UNSIGNED | ATCOMMAND_INTFLAGS_NOTATION_DEC,
			(i == (*valueGetBlocksCountP-1)) ? ATCOMMAND_STRING_TERMINATE : ATCOMMAND_ARGUMENT_DELIM))
		{
			return false;
		}
...
static bool DaphnisI_GPIO_ParseStatusBlocks(char *string, DaphnisI_GPIOStatusBlock_t *statusBlocksP, uint8_t *statusBlocksCountP)
{
	...
	*statusBlocksCountP = ATCommand_CountArgs(string);
	...
	for(uint8_t i=0; i<*statusBlocksCountP; i++)
	{
		...
		if(!ATCommand_GetNextArgumentInt(&string, &(statusBlocksP[i].GPIO_ID),
			ATCOMMAND_INTFLAGS_SIZE8 | ATCOMMAND_INTFLAGS_UNSIGNED | ATCOMMAND_INTFLAGS_NOTATION_DEC,
			GPIO_PIN_DELIMITER))
		{
			return false;
		...
		if(!ATCommand_GetNextArgumentInt(&string, &(statusBlocksP[i].status),
			ATCOMMAND_INTFLAGS_SIZE8 | ATCOMMAND_INTFLAGS_UNSIGNED | ATCOMMAND_INTFLAGS_NOTATION_DEC,
			(i == (*statusBlocksCountP-1)) ? ATCOMMAND_STRING_TERMINATE : ATCOMMAND_ARGUMENT_DELIM))
		{
			return false;
		}
...
static bool DaphnisI_GPIO_CreateConfigBlocksString(char *string, DaphnisI_GPIOConfigBlock_t *configBlocksP, uint8_t configBlocksCount)
{
	...
	{
		if (!ATCommand_AppendArgumentInt(string, configBlocksP[i].GPIO_ID,
			(ATCOMMAND_INTFLAGS_NOTATION_DEC | ATCOMMAND_INTFLAGS_SIZE8 | ATCOMMAND_INTFLAGS_UNSIGNED),
			GPIO_PIN_DELIMITER))
		{
			return false;
		...
		if (!ATCommand_AppendArgumentInt(string, configBlocksP[i].function,
			(ATCOMMAND_INTFLAGS_NOTATION_DEC | ATCOMMAND_INTFLAGS_SIZE8 | ATCOMMAND_INTFLAGS_UNSIGNED),
			ATCOMMAND_STRING_TERMINATE))
		{
			return false;
		...
		switch(configBlocksP[i].function)
		{
			...
			case DaphnisI_GPIO_IO_Input:
			{
				...
				if (!ATCommand_AppendArgumentInt(string, configBlocksP[i].value.input,
					(ATCOMMAND_INTFLAGS_NOTATION_DEC | ATCOMMAND_INTFLAGS_SIZE8 | ATCOMMAND_INTFLAGS_UNSIGNED),
					ATCOMMAND_STRING_TERMINATE))
				{
					return false;
			...
			case DaphnisI_GPIO_IO_Output:
			{
				...
				if (!ATCommand_AppendArgumentInt(string, configBlocksP[i].value.output,
					(ATCOMMAND_INTFLAGS_NOTATION_DEC | ATCOMMAND_INTFLAGS_SIZE8 | ATCOMMAND_INTFLAGS_UNSIGNED),
					ATCOMMAND_STRING_TERMINATE))
				{
					return false;
...
static bool DaphnisI_GPIO_CreateValueSetBlocksString(char *string, DaphnisI_GPIOValueSetBlock_t *valueSetBlocksP, uint8_t valueSetBlocksCount)
{
	...
	{
		if (!ATCommand_AppendArgumentInt(string, valueSetBlocksP[i].GPIO_ID,
			(ATCOMMAND_INTFLAGS_NOTATION_DEC | ATCOMMAND_INTFLAGS_SIZE8 | ATCOMMAND_INTFLAGS_UNSIGNED),
			GPIO_PIN_DELIMITER))
		{
			return false;
		...
		if (!ATCommand_AppendArgumentInt(string, valueSetBlocksP[i].value,
			(ATCOMMAND_INTFLAGS_NOTATION_DEC | ATCOMMAND_INTFLAGS_SIZE8 | ATCOMMAND_INTFLAGS_UNSIGNED),
			ATCOMMAND_STRING_TERMINATE))
		{
			return false;
