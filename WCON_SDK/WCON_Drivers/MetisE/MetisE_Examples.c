...
{
	uint16_t i = 0;
	printf("Received wmBus frame in mode %s with frame format %s", modeString[receivedData.wmBusModeRx], frameFormatString[receivedData.wmBusModeFrameFormat-1]);
	if(receivedData.rssi != METIS_E_RSSIINVALID)
	{
		printf(" with %d dBm", receivedData.rssi);
	}

	if(receivedData.timestamp != 0)
	{
		printf(" at %lu ms", receivedData.timestamp/METIS_E_CLOCK_KHZ);
	}

	printf(":\r\n0x");
	for (i = 0; i < receivedData.payloadLength; i++)
	{
		printf("%02x ", *(receivedData.pPayload + i));
	}
	fflush(stdout);
}
...
void MeterExample(void)
{
	if(!OpenConnection())
	{
		/* Could not init connection to module */
		return;
	}
