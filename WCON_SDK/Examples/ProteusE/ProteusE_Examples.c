...
{
	ProteusE_pins.ProteusE_Pin_Reset.port = (void*) GPIOA;
	ProteusE_pins.ProteusE_Pin_Reset.pin = GPIO_PIN_10;
	ProteusE_pins.ProteusE_Pin_BusyUartEnable.port = (void*) GPIOB;
	ProteusE_pins.ProteusE_Pin_BusyUartEnable.pin = GPIO_PIN_8;
	ProteusE_pins.ProteusE_Pin_Mode.port = (void*) GPIOA;
	ProteusE_pins.ProteusE_Pin_Mode.pin = GPIO_PIN_8;
	ProteusE_pins.ProteusE_Pin_StatusLed1.port = (void*) GPIOB;
...
static void TransparentModeExample()
{
	...
	while (1)
	{
		...
		statusPinState = ProteusE_GetStatusPinLed1Level();
		if (per_channelOpen)
		{
			if (!statusPinState)
			{
			...
			else if (false == ProteusE_IsTransparentModeBusy())
			{
				/* module not busy */
				...
				if (ProteusE_Transparent_Transmit(echo, sizeof(echo)))
				{
					...
					while (count < 20)
					{
						...
						WE_Delay(2);
						count++;
						if (true == ProteusE_IsTransparentModeBusy())
						{
							/* high detected */
							break;
						}
					...
					while (count < 128)
					{
						...
						WE_Delay(1);
						count++;
						if (false == ProteusE_IsTransparentModeBusy())
						{
							/* low detected */
							break;
						}
...
static void SleepCallback()
{
	WE_DEBUG_PRINT("Will go to sleep now\r\n");
}
...
static void GpioWriteCallback(bool remote, uint8_t gpioId, uint8_t value)
{
	WE_DEBUG_PRINT("GPIO write indication received (remote: %s, GPIO: %u, value: %u)\r\n", remote ? "true" : "false", gpioId, value);
}
