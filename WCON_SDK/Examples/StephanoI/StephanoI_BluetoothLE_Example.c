 ...
 */
StephanoI_ATBluetoothLE_Peripheral_DiscoverService_t StephanoI_Examples_Peripheral_DiscoverService = {0};
 ...
 */
StephanoI_ATBluetoothLE_MTU_t StephanoI_Examples_MTU = {.conn_index = BLE_CONNECTION_INDEX_INVALID, .MTU = BLE_DEFAULT_MTU};

/**
 * @brief Contains information on last write event (if any)
 */
StephanoI_ATBluetoothLE_Write_t StephanoI_Examples_Write = {.conn_index = BLE_CONNECTION_INDEX_INVALID};

/**
 * @brief Contains information on last read event (if any)
 */
StephanoI_ATBluetoothLE_Read_t StephanoI_Examples_Read = {.conn_index = BLE_CONNECTION_INDEX_INVALID};
...
StephanoI_ATBluetoothLE_EncryptionRequest_t StephanoI_Examples_EncryptionRequest = {.conn_index = BLE_CONNECTION_INDEX_INVALID};

bool received_securitykey_request = false;
StephanoI_ATBluetoothLE_SecurityKeyRequest_t StephanoI_Examples_SecurityKeyRequest = {.conn_index = BLE_CONNECTION_INDEX_INVALID};

bool received_confirmsecuritykey_request = false;
StephanoI_ATBluetoothLE_NotifyKey_t StephanoI_Examples_ConfirmSecurityKeyRequest = {.conn_index = BLE_CONNECTION_INDEX_INVALID};
...
#pragma message("Radio module must be configured to 3MBaud with flow control active, first")
    ...
    while (1)
    {
        ...
        else if (StephanoI_Examples_Connection.channelopen == false)
        {
            ...
            StephanoI_ATBluetoothLE_Central_ConnectionParameters_t conn_params = {.conn_index = StephanoI_Examples_Connection.conn_index, .min_interval = 6, .max_interval = 6, .latency = 0, .timeout = 400};
            ...
            StephanoI_ATBluetoothLE_MTU_t mtu = {.conn_index = StephanoI_Examples_Connection.conn_index, .MTU = 512};
...
#define nENABLE_CENTRAL_SECURITY
    ...
    while (1)
    {
        ...
        else if (StephanoI_Examples_Connection.channelopen == false)
        {
            ...
            StephanoI_ATBluetoothLE_MTU_t mtu = {.conn_index = StephanoI_Examples_Connection.conn_index, .MTU = PROTEUS_MAX_MTU};
            ret = StephanoI_ATBluetoothLE_CentralSetMTU(mtu);
            StephanoI_Examples_Print("Set MTU", ret);
            ret = StephanoI_ATBluetoothLE_SetConnectionPhy(StephanoI_Examples_Connection.conn_index, StephanoI_ATBluetoothLE_Phy_2Mbit);
            StephanoI_Examples_Print("Set Phy", ret);
...
void StephanoI_BluetoothLE_Peripheral_Advertising_Example()
{
    ...
    StephanoI_ATBluetoothLE_AdvertisingParameters_t adv_params = {.min_interval = 50, .max_interval = 50, .type = StephanoI_ATBluetoothLE_AdvertisingType_DIRECT_IND_LOW, .addr_type = StephanoI_ATBluetoothLE_Address_Public, .channel = StephanoI_ATBluetoothLE_ChannelMap_ALL, .filter = StephanoI_ATBluetoothLE_AdvertisingFilterPolicy_SCAN_ANY_CON_WLST};
    ...
    StephanoI_ATBluetoothLE_AdvertisingData_t adv_data = {.devname = "Stephano", .uuid = BLE_WE_SPP_PRIMARYSERVICE_UUID, .manufacturerdata = "00", .include_power = false};
