...
void Skoll_I_Examples(void)
{
	...
	Skoll_I_pins.Skoll_I_Pin_Reset.port = (void*) GPIOA;
	Skoll_I_pins.Skoll_I_Pin_Reset.pin = GPIO_PIN_10;
	Skoll_I_pins.Skoll_I_Pin_SPP.port = (void*) GPIOA;
	Skoll_I_pins.Skoll_I_Pin_SPP.pin = GPIO_PIN_7;
	Skoll_I_pins.Skoll_I_Pin_Connection.port = (void*) GPIOA;
	Skoll_I_pins.Skoll_I_Pin_Connection.pin = GPIO_PIN_9;
	Skoll_I_pins.Skoll_I_Pin_Role.port = (void*) GPIOA;
	Skoll_I_pins.Skoll_I_Pin_Role.pin = GPIO_PIN_8;
...
static void Test_Compiler_Packing()
{
	...
	uint8_t test_compiler_packing[] = {
			...
			0x0C,
			0x02,
			0x1A, /* system_get_uart_parameters response header (0x1A020CC0 little-endian) */
			0x11,
			0x22, /* result = 0x2211 test value */
			0x00,
			0xC2,
			0x01,
			0x00, /* baud = 0x0001C200 (115200) test value */
	...
	if (test_packet->payload.rsp_system_get_uart_parameters.result != 0x2211 || test_packet->payload.rsp_system_get_uart_parameters.baud != 0x0001C200 || test_packet->payload.rsp_system_get_uart_parameters.autobaud != 0xAA || test_packet->payload.rsp_system_get_uart_parameters.autocorrect != 0xBB || test_packet->payload.rsp_system_get_uart_parameters.flow != 0xCC || test_packet->payload.rsp_system_get_uart_parameters.databits != 0xDD
			|| test_packet->payload.rsp_system_get_uart_parameters.parity != 0xEE || test_packet->payload.rsp_system_get_uart_parameters.stopbits != 0xFF)
	{
